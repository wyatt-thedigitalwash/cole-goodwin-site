...
export default function CookieConsent() {
  ...
  // First-time visitors don't get the banner until the splash is out of the
  // way -- otherwise it (and the TermsGate popup that follows) would stack on
  // top of the splash overlay.
  useEffect(() => {
                  ...
                  {cat.locked ? (
                    ...
                    <button
                      ...
                      className={`mt-1 flex h-6 w-11 shrink-0 items-center rounded-full px-0.5 transition-colors ${
                        draft[cat.key] ? "bg-rust justify-end" : "bg-text-body/25 justify-start"
                      }`}
                    >
            ...
            <div className="mt-6 min-h-[20px]">
              {dirty && (
                <p className="flex items-center gap-2 font-body text-xs text-text-accent">
                  <span className="inline-block h-2 w-2 shrink-0 rounded-full bg-rust" />
